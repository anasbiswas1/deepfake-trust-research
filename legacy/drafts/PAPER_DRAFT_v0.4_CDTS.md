...
**Draft v0.4 (CDTS multi-component)** — github.com/anasbiswas1/deepfake-trust-research

> STATUS: 3 of 5 components experimentally complete (a Calibration, b Calibration Equity,
> d Label-free competence monitoring). Components (c) Explanation Stability and (e) Zero-Trust
> Routing pending. The unifying thesis is REVISED from the original plan (see §1.3, §8).
...
3. **(C3) Label-free competence monitoring** (Section 6): score-distribution-divergence signals
   predict competence without labels, and via the coupling, detect calibration risk on unseen
   generators (ROC-AUC 0.86).
4. **(C4) The CDTS wrapper and the GTD framing** unifying the above (Sections 3, 8).
...
## 7. Component (c): Explanation Stability [PENDING]

[Planned: SHAP / Grad-CAM / Integrated Gradients faithfulness (AOPC comprehensiveness/sufficiency,
deletion/insertion AUDC with NAOPC normalization) and rank-stability (Kendall-tau, RBO) under
degradation (c23/c40 compression, noise, blur). Extends Gowrisankar & Thing 2024. HYPOTHESIS
(competence-coupled): explanation faithfulness/stability degrades with competence, mirroring
calibration -- i.e. the same organizing principle. To be tested.]

---

## 8. The Unifying Principle: Competence Couples the Trust Signals [PARTIAL]

[The original plan hypothesized that calibration, explanation, and fairness signals co-fire
*temporally* when a new generator appears. Our results revise this: the signals are unified by
**competence**, not time. Established so far: calibration (§4) and calibration-equity (§5) and
label-free monitoring (§6) are all governed by competence. Component (c) will test whether
explanation stability is too. The unifying experiment: across the 21-generator set, correlate
(calibration error, explanation instability, subgroup-equity gap) and show they share a common
competence factor -- a single latent 'competence' explaining joint trust degradation. PENDING (c).]
...
### 10.1 Competence is the hidden variable of detector trustworthiness
Across calibration, equity, and monitoring, a single factor -- discriminative competence --
governs when the trust signal can be believed. This unifies phenomena previously studied
separately and gives a single deployable lever: estimate competence (label-free if needed) and
condition trust accordingly.
...
- Fig 1 coupling_competence_calibration.png; Fig 2 coupling_with_FRmismatch.png;
  Fig 3 (headline) coupling_full_grid.png; Fig 4 labelfree_detection_roc.png;
  Fig 5 timeline_df40_competence_calibration.png
