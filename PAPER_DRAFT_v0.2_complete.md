...
**Draft v0.2 (complete)** — github.com/anasbiswas1/deepfake-trust-research
...
Calibration Error, ECE) rises sharply as discriminative performance (AUC) falls. Across 24
...
1. We document a **competence–calibration coupling** in deepfake trust scores — calibration
   error rises as detection competence falls (Pearson r between −0.8 and −0.94 across slices)
...
### 5.4 Architecture-generality

To test whether the coupling is specific to Xception, we repeat the FF++ analysis with
EfficientNet-B4, a different architecture. It exhibits the same relationship (Pearson r = −0.95,
p = 0.05 over four configurations), its points falling on the Xception trend rather than forming
a separate one. Pooling all configurations — two architectures, two source datasets, four
forgery families, three checkpoint conditions, 24 points in total — yields a strong, highly
significant coupling: Pearson r = −0.81 (p = 1.5×10⁻⁶), Spearman ρ = −0.84. Table 3 summarizes
the correlation within each slice and combined, and Figure 3 is the headline scatter.

*[Figure 3 (headline): 24 points colored by architecture, OLS fit, r/p annotated.
File: figures/coupling_two_backbones.png]*
*[Table 3: correlation per slice (FF++ Xception; DF40 FS-only; DF40 within-dataset; EfficientNet
FF++; combined) with n, Pearson r/p, Spearman ρ/p, AUC range.
...
- **Figure 3 (headline)** — 24 points by architecture (figures/coupling_two_backbones.png)
- **Table 1** — pooled ECE_raw/ECE_cal/Brier per checkpoint
- **Table 2** — per-(detector, generator) AUC, ECE_raw, ECE_cal (all 24 configurations)
